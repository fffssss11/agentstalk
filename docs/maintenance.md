...
| `python scripts/maintain.py build` | 生成 `dist/release-版本号/`：源码 ZIP、`.sha256`、`SHA256SUMS.txt`、`RELEASE-NOTES.md` | 写 `dist/`，拒绝覆盖 |
...
标签固定为 `v` 加 `VERSION` 的内容，例如 `v0.2.0`。带 `-` 的版本由工作流自动标成 pre-release。已发布的标签不再移动；发布后发现问题，发布新的修订号。

## 发布步骤

1. `python scripts/maintain.py status`，确认清单一致，对照文件变化补全 CHANGELOG。
2. `python scripts/maintain.py check --browser`。
3. `python scripts/maintain.py bump 0.2.0` 预览，确认后加 `--apply`。
...
6. `python scripts/maintain.py build`，然后 `python scripts/maintain.py verify dist/release-0.2.0/agentstalk-0.2.0.zip --tests`。
...
9. 审阅暂存差异后，由你提交、打标签并推送：
   ...
   ```sh
   git -C .runtime/github-publish/agentstalk commit -m "Release 0.2.0"
   git -C .runtime/github-publish/agentstalk tag v0.2.0
   git -C .runtime/github-publish/agentstalk push origin HEAD v0.2.0
   ```

10. GitHub Actions「Release」依次核对标签与 `VERSION`、运行全部测试和浏览器测试、构建并验证干净副本。随后在只有写权限的独立任务中重新构建，确认 ZIP 的 SHA-256 与已测试的构建一致，再创建草稿 Release。
11. 在 GitHub 草稿中核对说明和附件。需要附加演示材料时，先放进本地 `dist/release-0.2.0/`，运行 `python scripts/maintain.py sums dist/release-0.2.0` 重算校验值，再上传材料并替换校验文件：`gh release upload v0.2.0 材料文件 dist/release-0.2.0/SHA256SUMS.txt --clobber`。确认后点 Publish。
...
- 草稿阶段 CI 失败：修复后，如草稿从未发布，可删除草稿和远端标签（`gh release delete v0.2.0 --cleanup-tag`）再重新打标签；已发布的版本不移动标签，改发修订号。
